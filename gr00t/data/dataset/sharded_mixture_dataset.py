...
class ShardedMixtureDataset(IterableDataset):
    ...
    def __init__(
        ...
        # Per-device batch size. Sequential mode needs it to hold each batch slot on
        # its own demonstration; see `_order_for_batch_slots`.
    ...
    def _order_for_batch_slots(self, run_lengths: list[int]) -> np.ndarray:
        """Order a shard so batch slot i advances one anchor per training iteration.

        The DataLoader cuts the stream into batches of `batch_size` consecutive
        items, so yielding a shard in plain order puts consecutive anchors of one
        demonstration side by side *within* a batch: slot i then jumps ahead by
        batch_size anchors between iterations.

        Instead, deal the shard's per-episode runs onto `batch_size` slot streams
        (longest run to the emptiest slot) and emit the m round-robin. Slot i then
        holds one demonstration and steps to its immediately following anchor on
        the next iteration, which is what the memory window assumes.

        Returns indices into the flat datapoint list of the shard.
        """
        runs, start = [], 0
        for length in run_lengths:
            runs.append(np.arange(start, start + length))
            start += length

        if self.batch_size == 1 or len(runs) < self.batch_size:
            # Fewer demonstrations than slots: no way to give each slot its own,
            # so fall back to plain sequential order.
            return np.concatenate(runs) if runs else np.arange(0)

        # logger.info(f"Ordering shard with {len(runs)} runs into {self.batch_size} batch slots")

        slots: list[list[np.ndarray]] = [[] for _ in range(self.batch_size)]
        slot_lengths = np.zeros(self.batch_size, dtype=int)
        for run in sorted(runs, key=len, reverse=True):
            target = int(np.argmin(slot_lengths))
            slots[target].append(run)
            slot_lengths[target] += len(run)

        streams = [np.concatenate(s) for s in slots]
        # Slots are balanced but rarely equal; the ragged tail can't form a full
        # batch without mixing slots, so it is left for the next shard's epoch.
        usable = min(len(s) for s in streams)
        if usable == 0:
            return np.concatenate(runs)
        return np.stack([s[:usable] for s in streams], axis=1).ravel()
    ...
    def __iter__(self):
        ...
        if self.sequential_anchors and self.num_workers > 1:
            # The DataLoader round-robins whole batches across workers, and each worker
            # reads a disjoint set of shards. Batch slot i is then only continuous every
            # num_workers-th iteration, not every iteration.
            warnings.warn(
                f"sequential_anchors=True with dataloader_num_workers={self.num_workers}: "
                "consecutive iterations are served by different workers reading different "
                "shards, so batch slot i at iteration t+1 does NOT follow slot i at "
                "iteration t. Use dataloader_num_workers<=1 for strictly consecutive anchors.",
                stacklevel=2,
            )
        ...
        while True:
            ...
            # Yield timesteps from current shard (shuffled unless sequential mode,
            # which preserves the temporal order the shards were built in).
            ...
            if not self.sequential_anchors: 
                ...
                indices_in_shard = self._order_for_batch_slots(
                    self.datasets[dataset_index].get_shard_run_lengths(shard_index)
                )
