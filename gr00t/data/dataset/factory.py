    ...
    def build(
        ...
        return (
            ShardedMixtureDataset(
                ...
                sequential_anchors=self.config.data.sequential_anchors,
                # Mirrors per_device_train_batch_size in experiment.run() -- this is the
                # batch size the DataLoader will actually cut the stream into.
                batch_size=(
                    self.config.training.global_batch_size // self.config.training.num_gpus
                    if self.config.training.batch_size is None
