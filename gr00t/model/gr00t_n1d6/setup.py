    ...
    def _create_model(self):
        ...
        if self.config.training.start_from_checkpoint is not None and not skip_weight_loading:
            model, loading_info = AutoModel.from_pretrained(
                ...
                # these two kwargs the loaded base-checkpoint defaults (False / 0.2)
                # silently win over whatever the run script requested.
                use_key_moment_gate=self.config.model.use_key_moment_gate,
                delta_threshold=self.config.model.delta_threshold,
